...
def quick_sort(demo, left, right): # 輸入資料，和從兩邊開始的位置
    ...
    i = left                      # 左指標
    j = right                     # 右指標
    key = demo[left]              # 基準點應擺的位置

    while i != j:
        while demo[j] > key and i < j:   # 從右邊開始找，找比基準點小的值
            j -= 1
        while demo[i] <= key and i < j:  # 從左邊開始找，找比基準點大的值
            i += 1
    ...
    quick_sort(demo, left, i-1)   # 繼續處理較小部分的子循環
    quick_sort(demo, i+1, right)  # 繼續處理較大部分的子循環
